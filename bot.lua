...
local function deowner_reply(extra, result, success)
  ...
  local ch = result.chat_id_
  redis:del('owners:'..ch)
  tdcli.sendText(result.chat_id_, 0, 0, 1, nil, '🚀 #Done\nuser '..user..' Is Not The Group *Owner* Anymore.', 1, 'md')
  print(user)
end
...
local function demute_reply(extra, result, success)
  ...
  tdcli.sendText(result.chat_id_, 0, 0, 1, nil, 'user '..result.sender_user_id_..' removed from *mutelist*.', 1, 'md')
end
  ...
  if (data.ID == "UpdateNewMessage") then
    ...
    if msg.content_.ID == "MessageText" then
      ...
      if input:match('^[!#/]([Oo]wner)$') then
        ...
        if owner == nil then
          tdcli.sendText(chat_id, 0, 0, 1, nil, "🔸This Group Hasn't Got Any *Owner* ", 1, 'md')
        end
      ...
      if input:match('^[/!#]delowner (.*)') and is_sudo(msg) then
        redis:del('owners:'..chat_id)
        tdcli.sendText(chat_id, 0, 0, 1, nil, 'user '..input:match('^[/!#]delowner (.*)')..' Is Not The Group *Owner* Anymore.', 1, 'md')
      end
      -----------------------------------------------------------------------------------------------------------------------
